...
async function getTranscript(link) {
  ...
  try {
    ...
    await driver.wait(
      until.elementLocated(By.xpath('//*[@id="expand"]')),
      30000,
    );
    ...
    let transcriptElements = await driver.findElements(
      By.className("segment style-scope ytd-transcript-segment-renderer"),
    );
    for (let i = 0; i < transcriptElements.length; i++) {
      let line = formatTranscript(
        await transcriptElements[i].getAttribute("aria-label"),
      );
