...
function clearConversationHistory() {
  ...
  fetch(`${CONFIG.API_ENDPOINT}/clearConversationHistory`, {
    ...
    .catch((error) => {
      console.error(
        "Error clearing conversation history on the server:",
        error,
      );
    });
}
...
function computeWeb() {
  ...
  let prompt =
    "I am going to pass in text data about a given YouTube video and I will ask you questions from the data";
...
function fetchGPT(prompt) {
  fetch(`${CONFIG.API_ENDPOINT}/askGPT`, {
    ...
    body: JSON.stringify({
      prompt:
        "I am now going to pass in metadata associated with a youtube video. I do not need any information back on this please respond 'ok got it' to this query" +
        prompt,
    }),
  })
    .then((response) => response.json())
...
function handleChatSubmit() {
  ...
  fetch(`${CONFIG.API_ENDPOINT}/askGPT`, {
    ...
    .then((response) => response.json())
    .then((data) => {
      const gptMessageContainer = document.querySelector(
        ".gpt-message-container:last-child",
      );
      if (gptMessageContainer) {
        gptMessageContainer.remove();
      }

      const updatedTypingIndicator =
        document.querySelector(".typing-indicator");
      const updatedGptMessage = document.querySelector(
        ".gpt-message:not(.typing-indicator)",
      );
